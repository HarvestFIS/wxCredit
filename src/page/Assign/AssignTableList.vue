<template>
    <div class="wxHome">
        <van-nav-bar class="dark_header" title="授信列表" fixed :z-index="999" />
            ...
            <van-list v-model="loading" :finished="finished" @load="onLoad">
                <van-cell v-for="item in list" v-bind:key="item.no" class="home-item-content">
                    ...
                    <span>
                        ...
                        <router-link :to="{ name: 'Assigindetails', query: { type: 'look', id: item.id }}">查看</router-link>
                    </span>
                </van-cell>
            </van-list>
        </div>
...
import {findListZhipai} from "api/api"
...
export default {
    ...
    methods: {
        onSearch() {
            ...
            findListZhipai(params).then((result) => {
                Toast.clear()
        ...
        onLoad() {
            // 初始化页数
            // this.page = 1
            findListZhipai({page: this.page}).then((result) => {
                let data = result.data
                for(let i = 0; i < data.rows.length; i++) {
                    this.list.push(data.rows[i])
                }
